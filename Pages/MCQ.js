...
export class MCQ {
  constructor(page) {
    ...
    this.MCQAddChoice = page.locator("(//button[@class='text-[#6937d3] my-6 text-sm font-medium py-2 px-3 border border-[#C4B4E4] border-dashed flex items-center justify-center rounded-[24px] ml-[30px]'])[1]");

    // Settings fields.
    this.S_MultipleSelection = page.locator(
      "//span[normalize-space()='Multiple Selection']"
    );

    // Explanation True.
    this.S_Explain = page.locator(
      "//span[normalize-space()='Explanation Field']"
    );
    this.S_Explain_minchar = page.locator("(//input[@value='0'])[1]");
    this.S_Explain_maxchar = page.locator("(//input[@value='0'])[2]");
    this.S_Explain_Field1 = page.locator(
      "(//input[contains(@placeholder,'Add your Question here')])[1]"
    );
    this.S_Explain_Field2 = page.locator(
      "(//input[contains(@placeholder,'Add your Question here')])[2]"
    );
    this.S_Explain_Field3 = page.locator(
      "(//input[contains(@placeholder,'Add your Question here')])[3]"
    );
    this.S_Explain_Field_Cancel = page.locator(
      "(//img[contains(@alt,'image')])[1]"
    );

    // Description.
    this.S_Description = page.locator(
      "//span[normalize-space()='Description Field']"
    );
    this.S_Description_field = page.locator(
      "(//div[contains(@class,'flex flex-col mb-[71.5px] gap-4 relative mr-[100px] ml-[88px] mt-[44px]')])[1]"
    );
    this.S_Description_minChar = page.locator("(//input[@value='0'])[1]");
    this.S_Description_maxChar = page.locator("(//input[@value='0'])[1]");
    this.S_Description_placeHolder = page.locator(
      "//input[@placeholder='Describe your answer...']"
    );

    // Others.
    this.S_Other = page.locator("//span[normalize-space()='Other']");
    this.S_Other_Inputfiled = page.locator("(//input[@value='Other'])");

    // None Option.
    this.S_None = page.locator("//span[normalize-space()='None']");
    this.S_None_option = page.locator("(//input[@value='Other'])");
  ...
  async Explain_field_3(text) {
    await this.S_Explain_Field3.fill(text);
  }
  ...
  async Option_Other (text){
    await this.S_Other_Inputfiled.fill(text);
  }

  async Option_Other_Visible (){
    await this.S_Other_Inputfiled.click();
  }

  async None_option_visible (){
    await this.S_None_option.isVisible();
  }
